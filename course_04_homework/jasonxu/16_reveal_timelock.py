# 使用 script3花费之前三脚本生成的 Taproot 地址资金
...
def main():
   ...
   # Alice 的内部私钥（用于 Taproot 地址）
    ...
    # 重建脚本树, script2存在问题，已生成地址里的资金只能用key path和script1, script3花费
    ...
    script2 = Script(
        ['OP_2', 
         ...
         bob_pub.to_hex(),
         'OP_2', 
         'OP_CHECKMULTISIG'] # not supported in tapscript
    )
    print(f"2nd script str:{script2.to_hex()}")

    # Script 3: CSV timelock
    relative_blocks = 200 # 200 blocks on testnet3, 需要几十分钟或更长时间解锁
    ...
    # 输入信息
    commit_txid = "b782fef34dabbd154c8d43e17d458933a81de626163ebdce719843eb6eb03345" 
    print(f"input txid: {commit_txid}")
    vout = 0
    input_amount = 9500  
    output_amount = 800
    fee = 200
   ...
   # 构建交易
    ...
    #sigB = bob_priv.sign_input(tx, 0, script3)
    print(f"sig:{sigB}")

    tx.witnesses.append(
        TxWitnessInput([
            sigB,        # (sig str)
            script3.to_hex(),          # script 本体
