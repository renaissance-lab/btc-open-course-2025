...
def main():
    ...
    # 计算总输入金额
    total_input = legacy_amount + taproot_amount
    ...
    txinputs = [txin_segwit, txin_legacy]
    ...
    utxos_script_pubkeys = [segwit_script, legacy_script]
    amounts = [segwit_amount, legacy_amount]
    
    # 签名所有输入
    sig1 = from_private_key.sign_segwit_input(tx, 0, segwit_script, amounts[0])
    # sig3 = from_private_key.sign_taproot_input(tx, 2, [taproot_script], [taproot_amount])
    # sig2 = from_private_key.sign_taproot_input(tx, 0, [taproot_script], [taproot_amount])
    sig3 = from_private_key.sign_input(tx, 1, legacy_script)
    ...
    txinputs[1].script_sig = Script([sig3, from_pub.to_hex()])
    # txinputs[0].script_sig = Script([])
    
    # 设置见证数据（只为SegWit和Taproot设置）
    tx.witnesses = []  # 清空现有见证数据
    # tx.set_witness(0, TxWitnessInput([sig1, from_pub.to_hex()]))
    # tx.set_witness(1, TxWitnessInput([sig2]))
    # tx.witnesses.append(TxWitnessInput([sig1, from_pub.to_hex()]))
